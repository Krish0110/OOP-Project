	...
	for (auto t : tiles)
	{
		if (Map::startMapMovement == true)
		{
			...
			else if (updateCounter >= 1500)
			{
				...
				for (static bool run = true;run;run = false)
				{
...
void Game::render()
{
	...
	if (Collision::hitCount >= 5)
	{
		winLoseTexture = TextureManager::LoadTexture("gameLoop/gfx/winFinal.png");
		TextureManager::Draw(winLoseTexture, srcWinLose, destWinLose, SDL_FLIP_NONE);
	}
	if (updateCounter >= 1600 && Collision::hitCount < 5)
	{
		winLoseTexture = TextureManager::LoadTexture("gameLoop/gfx/loseFinal.png");
		TextureManager::Draw(winLoseTexture, srcWinLose, destWinLose, SDL_FLIP_NONE);
	}
